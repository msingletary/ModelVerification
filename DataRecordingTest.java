...
public class DataRecordingTest {
	...
	HashMap<String, Integer> getMapOfNameIndices(ArrayList<DataRecord> inputList) {
		int nameCount = -1;
		// or, could get a list of all the event names from the DSL
		// user needs to provdide these when determining what to record anyways
		HashMap<String, Integer> namesMap = new HashMap<String, Integer>();
		for (DataRecord dr : inputList) {
	...
	void generateMapOfName() {
		int nameCount = -1;
		// or, could get a list of all the event names from the DSL
		// user needs to provdide these when determining what to record anyways
		allVariableNames = new HashMap<String, Integer>();
		for (DataRecord dr : dataRecordList) {
			if (allVariableNames.get(dr.variableName) == null) {
				allVariableNames.put(dr.variableName, ++nameCount);
			}
		}
		// key: name of variable
		// value: index of this variable in the overall data array or matrix
	...
	// Does DataRecord need to record generic type? or can 
	//	void recordData(DataRecord<E> record, String methodEventname) {
	void recordData(String methodEventName, int data) {
		...
		//allVariableNames.put(methodEventName);
	}
	
	public DataRecordingTest() {
